...
int main() {
  ...
  while (*raw_ptr != 0xFFFFFFFF) {
    ...
    // Workaround for xil_printf float limitation
    float angle_deg = (float)(int16_t)kp->angle * 180.0f / 16384.0f;
    int deg_int = (int)angle_deg;
    int deg_frac = (int)(angle_deg * 10.0f) % 10;
    if (deg_frac < 0) deg_frac = -deg_frac;

    xil_printf("KP[%d]: X=%d, Y=%d, Angle=0x%04X (%d.%d deg), Score=%lu\r\n",
        kp_count, kp->x, kp->y, kp->angle,
        deg_int, deg_frac,
        kp->response);
